...
// الحالات التي تظهر في صفحة الدليفري — تشمل كل ما يُرسله الشيف
const DELIVERY_STATUSES = ["ready", "delivering", "out_for_delivery", "delivered"];
...
function normalizeOrder(id, data) {
  ...
  if (raw === "out_for_delivery"   || raw === "on_delivery") status = "delivering";
...
function render() {
  ...
  for (const order of visible) {
    ...
    } else if (order.status === "delivering" || order.status === "out_for_delivery") {
      actions = `<button class="success" data-action="deliver" data-id="${order.id}">✅ تم التسليم</button>`;
    } else {
      actions = `
        <button class="secondary" disabled>تم التسليم ✓</button>
        <button class="btn-delete" data-action="delete" data-id="${order.id}">🗑️ حذف</button>
      `;
    }
  ...
  document.getElementById("deliveryCount").textContent =
    orders.filter(o => o.status === "delivering" || o.status === "out_for_delivery").length;
...
async function changeStatus(id, status) {
  try {
    ...
    if (status === "delivering") update.deliveryStartedAt = firebase.firestore.FieldValue.serverTimestamp();
...
ordersGrid.addEventListener("click", e => {
  ...
  if (action === "start")   changeStatus(id, "delivering");
...
// ── مسح جميع المُسلَّمة دفعة واحدة ──────────────────────────
async function clearAllDelivered() {
  const delivered = orders.filter(o => o.status === "delivered");
  if (delivered.length === 0) {
    showToast("لا توجد طلبات مُسلَّمة للمسح");
    return;
  }
  if (!confirm(`هل تريد حذف ${delivered.length} طلب مُسلَّم نهائياً؟`)) return;
  ...
  try {
    ...
    await batch.commit();
    showToast(`✅ تم مسح ${delivered.length} طلب بنجاح`);
  } catch (err) {
    console.error(err);
    showToast("❌ خطأ أثناء المسح الجماعي");
  }
}
...
// بدون orderBy لتجنب Composite Index
db.collection("orders")
  ...
  .onSnapshot(snapshot => {
    orders = snapshot.docs
      ...
      .sort((a, b) => {
        const priority = { ready: 0, delivering: 1, out_for_delivery: 1, delivered: 2 };
    ...
    ordersGrid.innerHTML = `<div style="background:#fee2e2;border-radius:12px;padding:20px;color:#dc2626;font-weight:700;text-align:center;">
      ❌ تعذر الاتصال بقاعدة البيانات<br>
      <small style="font-weight:400;color:#7f1d1d">${err.message}</small>
    </div>`;
  });
